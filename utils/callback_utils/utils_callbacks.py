...
class CallBackLoggingList(object):
    def __init__(self, frequent, rank, total_step, batch_size, world_size, writer=None):
        self.frequent: int = frequent
        self.rank: int = rank
        self.time_start = time.time()
            ...
            if self.init:
                ...
                if self.writer is not None:
                    ...
                    for loss in loss_list:
                        self.writer.add_scalar('loss', loss.name, loss.avg, global_step)
                total_loss_msg = ""
                for loss in loss_list:
                    loss_msg = "{}: avg {:.4f}, current {:.4f}, ".format(loss.name, loss.avg, loss.val)
                    total_loss_msg += loss_msg
                    
                if fp16:
                    msg = "Speed %.2f samples/sec   Loss %s   LearningRate %.4f   Epoch: %d   Global Step: %d   " \
                          "Fp16 Grad Scale: %2.f   Required: %1.f hours" % (
                              speed_total, total_loss_msg, learning_rate, epoch, global_step,
                              grad_scaler.get_scale(), time_for_end
                          )
                else:
                    msg = "Speed %.2f samples/sec   Loss %.4f   LearningRate %.4f   Epoch: %d   Global Step: %d   " \
...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, backbone, partial_fc, prefix=""):
        if global_step > 100 and self.rank == 0:
            temp_save_path = "{}backbone-{}.pth".format(prefix, global_step)
