...
def run_train(opt, device_id, rank, world_size):
    torch.cuda.set_device(rank)
    seed = opt.utils.seed
    emb_size = opt.network.emb_size

    trainset = opt.dataset.trainset
    sample_rate = opt.network.sample_rate
    ...
    trainer = Trainer(device_id, rank, world_size, num_classes_list, num_samples_list, batch_size_list, 
            trainset, num_epoch, emb_size=emb_size, sample_rate=sample_rate)
    total_step = num_image // (batch_size_list[0] * num_epoch * world_size)
    print("total step: {}, num epoch: {}, batch_size: {}, sample_rate: {}, world size: {}, device_id: {}, emb_size: {}".format(total_step, num_epoch, batch_size, sample_rate, world_size, device_id, emb_size))
...
class Trainer():
    ...
    def __init__(self, device_id, local_rank, world_size, num_classes_list, num_samples_list, 
            batch_size_list, dataset_name_list, num_epoch, emb_size=512, sample_rate=0.1, loss_type="cosface", fp16=True):
    ...
    def network_init(self):
        self.backbone = iresnet.iresnet180(dropout=0.0, fp16=self.fp16, num_features=self.emb_size)
        self.backbone.train()
...
def main(args):
    ...
    run_train(opt, local_rank, current_device, world_size)
