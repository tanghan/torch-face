...
dataset_dict = {"lfw": ["/home/users/han.tang/data/public_face_data/glint/glint360k/lfw.bin"], 
        ...
        "ValID": ["/cluster_home/test_data/ValID/wanren_V0.2_indexed.rec",
                    "/cluster_home/test_data/ValID/wanren_V0.2_indexed.idx"],
        ...
        "Val_DMS_Car": ["/home/users/han.tang/data/test/val/Val_DMS_Car/DMS_Car.rec",
                    "/home/users/han.tang/data/test/val/Val_DMS_Car/DMS_Car.idx"],
        "abtdge_id1w": ["/cluster_home/data/train_data/abtdge_id1w/abtdge_id1w_Above9_miabove10_20200212.rec",
                    "/cluster_home/data/train_data/abtdge_id1w/abtdge_id1w_Above9_miabove10_20200212.idx"],
        }
...
def build_baseline_dataset(rec_path, idx_path, local_rank, batch_size, origin_prepro):
    ...
    dataloader = EvalDataLoader(
        local_rank=local_rank, dataset=dataset, batch_size=batch_size,
        sampler=sampler, num_workers=4, pin_memory=False, drop_last=False)
    return dataloader, total_num
...
def run(args, device_id, rank, world_size):
    ...
    output = args.output
    feature_path = "{}.bin".format(device_id)
    feature_path = os.path.join(output, feature_path)
    label_path = "{}.txt".format(device_id)
    ...
    write_label_index(label_list, device_id, label_index_path)
...
def main(args):
    ...
    gpu_num = args.gpu_num
    run(args, local_rank, current_device, world_size)
    '''
    size = gpu_num
    processes = []
