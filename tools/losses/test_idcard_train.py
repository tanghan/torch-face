...
def run_train(args, rank, world_size):
    ...
    dataloader = get_dataloader(rng, batch_size, num_classes, num_samples, input_shape)
    ...
    trainer = Trainer(rank, world_size, num_classes=num_classes, num_images=num_images, batch_size=batch_size, num_epoch=num_epoch, sample_rate=sample_rate, probe_weights_path=probe_weights_path, gallery_weights_path=gallery_weights_path, 
            backbone_lr_ratio=backbone_lr_ratio, resume=resume, loss_type=loss_type, fp16=fp16)
    ...
    for epoch in range(0, num_epoch):
        ...
        total_step = trainer.train(epoch, global_step, dataloader, grad_amp, loss, criterion) 
        global_step = total_step

def get_dataloader(rng, batch_size, num_classes, num_samples, input_shape):
    input_feat1 = rng.normal(size=(num_samples, input_shape[0], input_shape[1],input_shape[2]))
    input_feat2 = rng.normal(size=(num_samples, input_shape[0], input_shape[1],input_shape[2]))
    input_feat1 = torch.tensor(input_feat1, dtype=torch.float32)
    input_feat2 = torch.tensor(input_feat2, dtype=torch.float32)
...
class Trainer():
    ...
    def __init__(self, local_rank, world_size, num_classes, num_images, batch_size=128, emb_size=2048, num_epoch=12, 
            sample_rate=0.1, resume=True, probe_weights_path="./", gallery_weights_path=None, backbone_lr_ratio=1., loss_type="cosface", fp16=True):
        ...
        self.prototype = SST_Prototype(local_rank=local_rank, world_size=world_size, batch_size=batch_size, feat_dim=emb_size,
                queue_size=16384, scale=32.0, loss_type="am_softmax", margin=0.35).cuda(self.local_rank)
    ...
    def network_init(self):
        self.probe_backbone = iresnet.iresnet18(dropout=0.0, fp16=self.fp16, num_features=self.emb_size)
        self.gallery_backbone = iresnet.iresnet18(dropout=0.0, fp16=self.fp16, num_features=self.emb_size)
    ...
    def train(self, epoch, global_step, train_loader, grad_amp, loss,
        ...
        for step, (probe_imgs, gallery_imgs, labels) in enumerate(train_loader):
            ...
            probe_imgs = probe_imgs.cuda(self.local_rank)
            gallery_imgs = gallery_imgs.cuda(self.local_rank)
            ...
            output1, output2, label = self.prototype(
